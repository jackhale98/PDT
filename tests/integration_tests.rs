...
// ============================================================================
// Quote Lead Time for Quantity Tests
// ============================================================================

#[test]
fn test_quote_lead_time_for_qty() {
    let tmp = setup_test_project();

    create_test_component(&tmp, "PN-LEAD", "Lead Time Component");
    create_test_supplier(&tmp, "Lead Supplier");
    ...
    // Create quote with different lead times at different quantities
    tdt()
        .current_dir(tmp.path())
        .args([
            "quote",
            ...
            "Lead Time Quote",
            "--breaks",
            "1:10.00:7,100:8.00:14,1000:6.00:28",
            "--no-edit",
        ])
        .assert()
        .success();
    ...
    // Check lead time at qty 100 (should be 14 days)
    tdt()
        .current_dir(tmp.path())
        .args(["quote", "price", "QUOT@1", "--qty", "100"])
        .assert()
        .success()
        .stdout(predicate::str::contains("Lead Time: 14 days"));

    // Check lead time at qty 1000 (should be 28 days)
    tdt()
        .current_dir(tmp.path())
        .args(["quote", "price", "QUOT@1", "--qty", "1000"])
        .assert()
